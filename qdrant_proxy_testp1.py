...
# ── 2b: Chat File Attachments ──────────────────────────────────────────────────
ATTACH_DIR    = Path(__file__).parent / ".attachments"
ATTACH_MAX_SIZE = 1 * 1024 * 1024  # 1 MB
ATTACH_MAX_FILES = 5
ATTACH_ALLOWED_EXT = frozenset({
    ".txt", ".md", ".csv", ".json", ".xml", ".yaml", ".yml", ".log",
    ".py", ".js", ".ts", ".jsx", ".tsx", ".html", ".css", ".scss",
    ".sql", ".rb", ".go", ".java", ".rs", ".c", ".cpp", ".h", ".sh",
    ".bat", ".ps1", ".env", ".ini", ".cfg", ".toml",
})

_attachments: dict[str, dict] = {}

def _cleanup_old_attachments():
    """Remove attachment files not tracked in _attachments (orphans)"""
    if not ATTACH_DIR.exists():
        return
    kept_ids = set(_attachments.keys())
    for p in ATTACH_DIR.iterdir():
        if p.name not in kept_ids:
            try:
                p.unlink()
            except Exception:
                pass
...
class ProxyHandler(_SuppressBrokenPipe, BaseHTTPRequestHandler):
    ...
    def do_POST(self):
        ...
        if body and "messages" in body:
            ...
            if attachments and body.get("messages"):
                last_msg = body["messages"][-1]
                if last_msg.get("role") == "user":
                    parts = []
                    for fid in attachments:
                        meta = _attachments.get(fid)
                        if not meta:
                            continue
                        try:
                            text = Path(meta["path"]).read_text("utf-8", errors="replace")
                            parts.append(f"--- Attached file: {meta['name']} ({meta['size']} bytes) ---\n{text}\n--- End of attached file ---")
                        except Exception:
                            parts.append(f"--- Attached file: {meta['name']} (error reading) ---")
                    if parts:
                        preamble = "\n\n".join(parts)
                        last_msg["content"] = preamble + "\n\n" + last_msg["content"]
...
/* file attachments */
.attach-row{{display:flex;flex-wrap:wrap;gap:6px;min-height:0;padding:0 2px}}
.attach-chip{{display:inline-flex;align-items:center;gap:4px;background:var(--surf2);border:1px solid var(--border);border-radius:4px;padding:2px 7px;font-size:11px;color:var(--muted);max-width:100%}}
.attach-chip .aname{{overflow:hidden;text-overflow:ellipsis;white-space:nowrap;max-width:130px}}
.attach-chip .asize{{color:var(--accent);font-size:10px;white-space:nowrap}}
.attach-chip .arm{{background:none;border:none;color:var(--muted);cursor:pointer;font-size:15px;line-height:1;padding:0 2px}}
.attach-chip .arm:hover{{color:var(--red)}}
.attach-btn{{background:transparent;border:1px solid var(--border);border-radius:var(--r);color:var(--muted);cursor:pointer;font-size:18px;padding:5px 10px;height:38px;line-height:1.2;display:flex;align-items:center;flex-shrink:0}}
.attach-btn:hover{{color:var(--accent);border-color:var(--accent)}}
.attach-indicator{{color:var(--accent);font-size:10px;font-weight:600}}
...
<div class="main">
  ...
  <div class="right">
    ...
    <div class="chat-input-bar">
      ...
      <div class="input-row">
        <button class="attach-btn" id="attachBtn" onclick="attachFile()" title="Attach a file">📎</button>
...
async function attachFile() {{
  ...
  input.type = 'file';
  input.accept = '.txt,.md,.csv,.json,.xml,.yaml,.yml,.log,.py,.js,.ts,.jsx,.tsx,.html,.css,.sql,.rb,.go,.java,.rs,.c,.cpp,.h,.sh,.bat,.env,.ini,.cfg,.toml';
  input.onchange = async () => {{
    if (!input.files.length) return;
    const file = input.files[0];
    if (file.size > 1048576) {{ alert('File too large (max 1 MB)'); return; }}
    if (attachedFiles.length >= 5) {{ alert('Max 5 files per message'); return; }}
    const text = await file.text();
    try {{
      const resp = await fetch('/api/chat/attach', {{
        method: 'POST',
        headers: {{'Content-Type':'application/json'}},
        body: JSON.stringify({{name:file.name, content:text, session_id:sessionId}})
      }});
      const data = await resp.json();
      if (!resp.ok) {{ alert('Upload failed: ' + (data.error||'unknown')); return; }}
      attachedFiles.push({{file_id:data.file_id, name:file.name, size:file.size}});
      renderAttachments();
...
function renderAttachments() {{
  ...
  row.innerHTML = attachedFiles.map((f,i) =>
    '<span class="attach-chip"><span class="aname">' + esc(f.name) + '</span><span class="asize">' + fmtSize(f.size) + '</span><button class="arm" onclick="removeAttach('+i+')" title="Remove">&times;</button></span>'
  ).join('');
}}

function removeAttach(idx) {{
  const f = attachedFiles[idx];
  fetch('/api/chat/attach/' + f.file_id, {{method:'DELETE'}}).catch(()=>{{}});
...
async function sendMessage() {{
  ...
  const msgAttachments = attachedFiles.slice();
  ...
  appendMsg('user', text + (msgAttachments.length ? ' [+' + msgAttachments.length + ' file' + (msgAttachments.length>1?'s':'') + ']' : ''), '');
  ...
  try {{
    ...
    const body = {{model, messages: conversationHistory, stream:true, session_id: sessionId, ...genParams}};
    if (msgAttachments.length) body.attachments = msgAttachments.map(f => f.file_id);
...
class DashboardHandler(_SuppressBrokenPipe, BaseHTTPRequestHandler):
    ...
    def do_GET(self):
        ...
        if self.path == "/api/context/clear":
            ...
            self.wfile.write(resp)
            return
        if self.path.startswith("/api/chat/attach/"):
            file_id = self.path.split("/api/chat/attach/")[-1]
            meta = _attachments.get(file_id)
            if not meta:
                self.send_response(404)
                self.end_headers()
        ...
        if self.path == "/api/chat/attach":
            try:
                ...
                if not name or not content:
                    resp = json.dumps({"error": "name and content required"}).encode()
                ...
                if ext not in ATTACH_ALLOWED_EXT:
                    resp = json.dumps({"error": f"File type {ext} not allowed"}).encode()
                    self.send_response(400); self.send_header("Content-Type", "application/json")
                    self.send_header("Content-Length", str(len(resp))); self.end_headers(); self.wfile.write(resp); return
                encoded = content.encode("utf-8")
                if len(encoded) > ATTACH_MAX_SIZE:
                    resp = json.dumps({"error": f"File too large (max {ATTACH_MAX_SIZE//1024} KB)"}).encode()
                ...
                dest = ATTACH_DIR / file_id
                dest.write_bytes(encoded)
                _attachments[file_id] = {
                    "name": name, "path": str(dest), "size": len(encoded),
                    "session_id": session_id, "created_at": datetime.now().isoformat(),
                }
                resp = json.dumps({"file_id": file_id, "name": name, "size": len(encoded)}).encode()
    ...
    def do_DELETE(self):
        if self.path.startswith("/api/chat/attach/"):
            file_id = self.path.split("/api/chat/attach/")[-1]
            meta = _attachments.pop(file_id, None)
            if not meta:
                self.send_response(404)
                self.end_headers()
                self.wfile.write(b'{"error":"Not found"}')
...
if __name__ == "__main__":
    ...
    # Ensure attachment directory exists and purge orphans
    ATTACH_DIR.mkdir(parents=True, exist_ok=True)
    _cleanup_old_attachments()
